				...
				if (/^[-*]\s+/.test(nextNormalizedLine)) {
					...
					if (item.children.length > 0) {
						...
						item.children[item.children.length - 1] =
							`${item.children[item.children.length - 1]} ${nextNormalizedLine}`.trim();
...
function parseBlocks(content: string): Block[] {
	...
	while (i < lines.length) {
		...
		// 단독 이미지 블록 감지
		const imageMatch = normalizedLine.match(
			/^!\[([^\]]*)\]\(([^\s\)]+)(?:\s+"([^"]+)")?\)$/,
		);
		if (imageMatch) {
			blocks.push({
		...
		while (i < lines.length) {
			...
			if (
				!normalizedNextLine ||
				/^(##|###|```|>\s+|[-*]\s+|\d+\.\s+)/.test(normalizedNextLine)
			) {
				break;
			}
		...
		if (boldMatch) {
			return (
				<strong
					...
					className={css({ fontWeight: "800", color: "var(--text)" })}
				>
							...
							<ul key={`${block.type}-${index}`}>
								{block.items.map((item, itemIndex) => (
									<li key={`${item.text}-${itemIndex}`}>
										<div>
											{renderInline(item.text, `ul-${index}-${itemIndex}`)}
										</div>
																...
																<li key={childIndex}>
																	{renderInline(
																		subOrderedMatch
																			? subOrderedMatch[2]
																			: child,
																		`ul-${index}-${itemIndex}-child-${childIndex}`,
																	)}
																</li>
															);
														})}
						...
						return (
							<ol key={`${block.type}-${index}`}>
								{block.items.map((item, itemIndex) => (
									<li key={`${item.text}-${itemIndex}`}>
										<div>
											{renderInline(item.text, `ol-${index}-${itemIndex}`)}
										</div>
										{item.nestedText && item.nestedText.length > 0 && (
											<div
												className={css({
